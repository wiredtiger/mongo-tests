...
void *compressMultiZstdCCtx(void *arg) {
    ...
    ZSTD_CDict* dict = ZSTD_createCDict(NULL, 0, compressionLevel);
    ZSTD_CCtx *ctx = cctx_array[threadId].ctx;
    ...
    for (int j = threadId; j < nParts; j = j + nThreads) {
         ...
         */
        compressedSizes[j] = ZSTD_compress_usingCDict((ZSTD_CCtx *)(cctx_array[threadId].ctx),
                                        compressedParts[j], 
                                        chunkSize,
                                        uncompressedParts[j], 
...
int decompressZstd(const char *fileName, const char *outFileName, size_t nParts, size_t chunkSize, size_t compressedSizes[]) {
    ...
    for (int j = 0; j < nParts; j++) {
         ...
         * the last slot of file will be less than chunk size, hence take min.
         */
        ZSTD_decompressDCtx((ZSTD_DCtx *)(dctx_array[0].ctx),
                                    decompressedParts[j], 
                                    chunkSize,
                                    prevCompressed[j], 
                                    compressedSizes[j]);
