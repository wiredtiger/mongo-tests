...
ZSTD_CStream **cstream_array;
ZSTD_DStream **dstream_array;
...
static int 
zstd_init_stream_array(CONTEXT_TYPE ctx_type, int count) {
    if (ctx_type == CONTEXT_TYPE_COMPRESS)
        cstream_array = malloc(sizeof(ZSTD_CStream *) * nThreads);
    else
        dstream_array = malloc(sizeof(ZSTD_DStream *) * nThreads);

    for (int i = 0; i < nThreads; i++) {
        if (ctx_type == CONTEXT_TYPE_COMPRESS) {
            cstream_array[i] = (void *)ZSTD_createCStream();
            //size_t errorCode = ZSTD_CCtx_setParameter(cctx_array[i].ctx, ZSTD_c_nbWorkers, nThreads);
        }
        else
            dstream_array[i] = (void *)ZSTD_createDStream();
    }
...
void *compressMultiZstdCCtx(void *arg) {
    ...
    for (int j = threadId; j < nParts; j = j + nThreads) {
         ...
         */
        size_t const initResult = ZSTD_initCStream(cstream_array[threadId], compressionLevel);      
        if (ZSTD_isError(initResult)) {                                                             
            fprintf(stderr, "ZSTD_initCStream() error : %s \n", ZSTD_getErrorName(initResult));     
            exit(11);                                                                               
        }                                                                                           
        /* Input (to compress), number of bytes read, position (will always be 0 in our use case). */
        ZSTD_inBuffer input = {uncompressedParts[j], MIN(chunkSize, fileSize - j * chunkSize), 0};  
        ZSTD_outBuffer output = {compressedParts[j], chunkSize, 0};                                 
        ZSTD_compressStream(cstream_array[threadId], &output, &input);                              
        ZSTD_endStream(cstream_array[threadId], &output);                                           
        compressedSizes[j] = output.pos;        
...
int compressZstd(const char *fileName, const char *outFileName, int nParts, size_t chunkSize, size_t compressedSizes[]) {
    ...
    /* Each compression thread has their own context. */
    pthread_t tids[nThreads];
    zstd_init_stream_array(CONTEXT_TYPE_COMPRESS, nThreads);
...
int decompressZstd(const char *fileName, const char *outFileName, size_t nParts, size_t chunkSize, size_t compressedSizes[]) {
    ...
    zstd_init_stream_array(CONTEXT_TYPE_DECOMPRESS, 1);
    ...
    for (int j = 0; j < nParts; j = j + nThreads) {
         ...
         * the last slot of file will be less than chunk size, hence take min.
         */
       size_t const initResult = ZSTD_initDStream(dstream_array[0]);                        
                                                                                                    
        /* Input (to decompress), number of bytes read, position (will always be 0 in our use case). */
        ZSTD_inBuffer input = {prevCompressed[j], compressedSizes[j], 0};                           
        ZSTD_outBuffer output = {decompressedParts[j], chunkSize, 0};                               
        ZSTD_decompressStream(dstream_array[0], &output, &input);             
    } 
