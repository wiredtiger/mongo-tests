...
ZSTD_CONTEXT *cctx_array;
ZSTD_CONTEXT *dctx_array;
...
static int 
zstd_init_context_array(CONTEXT_TYPE ctx_type, int count) {
    if (ctx_type == CONTEXT_TYPE_COMPRESS)
        cctx_array = malloc(sizeof(ZSTD_CONTEXT) * nThreads);
    else
        dctx_array = malloc(sizeof(ZSTD_CONTEXT) * nThreads);

    for (int i = 0; i < nThreads; i++) {
        if (ctx_type == CONTEXT_TYPE_COMPRESS) {
            cctx_array[i].ctx = (void *)ZSTD_createCCtx();
            size_t errorCode = ZSTD_CCtx_setParameter(cctx_array[i].ctx, ZSTD_c_nbWorkers, nThreads);
        }
        else
            dctx_array[i].ctx = (void *)ZSTD_createDCtx();
    }
...
void *compressMultiZstdCCtx(void *arg) {
    ...
    void *compressCtx = ZSTD_createCCtx();
    ZSTD_CCtx_setParameter(compressCtx, ZSTD_c_nbWorkers, nThreads);
    ...
    for (int j = 0; j < nParts; j++) {
         ...
         */
        compressedSizes[j] = ZSTD_compressCCtx((ZSTD_CCtx *)(compressCtx),
                                        compressedParts[j], 
                                        chunkSize,
                                        uncompressedParts[j], 
...
int compressZstd(const char *fileName, const char *outFileName, int nParts, size_t chunkSize, size_t compressedSizes[]) {
    ...
    //zstd_init_context_array(CONTEXT_TYPE_COMPRESS, nThreads);

    //for (int i = 0; i < nThreads; i++) {
    {
	//Create only one app thread as zstd will parallelize the internal workers.
        int *threadId;
        threadId = malloc(sizeof(int));
        *threadId = 0;
        pthread_create(&tids[0], NULL, compressMultiZstdCCtx, threadId);
    }
...
int decompressZstd(const char *fileName, const char *outFileName, size_t nParts, size_t chunkSize, size_t compressedSizes[]) {
    ...
    void *decompressCtx = ZSTD_createDCtx();
    size_t bytesRead = 0;
    for (int i = 0; i < nParts; i++) {
    ...
    //zstd_init_context_array(CONTEXT_TYPE_DECOMPRESS, 1);
    ...
    for (int j = 0; j < nParts; j++) {
         ...
         * the last slot of file will be less than chunk size, hence take min.
         */
        ZSTD_decompressDCtx((ZSTD_DCtx *)(decompressCtx),
                                    decompressedParts[j], 
                                    chunkSize,
                                    prevCompressed[j], 
                                    compressedSizes[j]);
...
int main(int argc, const char **argv) {
    ...
    if (argc < 4)
        printf("Usage: %s nWorkers inputFile outputFile zstdCompressionLevel[optional]\n", exeName);
