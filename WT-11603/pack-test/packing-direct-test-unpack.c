...
main(int argc, char **argv)
{
    ...
    if (argc > 2 && !strcmp(argv[1], "gen")) {
        ...
        for (i = 4; i < argc; i++) {
            test_unpack_run(
              (WT_SESSION_IMPL *)g.wt_session, argv[3], argv[i], garbage, (size_t)garbage_size);
        }
        ...
        for (i = 2; i < argc; i++) {
            test_unpack_run(
              (WT_SESSION_IMPL *)g.wt_session, argv[1], argv[i], garbage, (size_t)garbage_size);
        }
    } else {
        printf(
          "Usage:\n"
          ...
          "optypes:\n%s",
          argv[0], argv[0], argv[0], test_list_ops());
    }
